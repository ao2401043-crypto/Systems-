...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName("embed_color")
        .setDescription("لون الأيمبد (مثال: #ff0000)"))
    .addStringOption(option =>
  ...
  async execute(interaction) {
    ...
    const embedColor = interaction.options.getString("embed_color") || "#2F3136";
    const embedTitle = interaction.options.getString("embed_title");
    const guildId = interaction.guild.id;

    // ربط الألوان مع ButtonStyle
    const colorMap = {
    ...
    try {
      ...
      await interaction.followUp({ content: "⌛ انتهى الوقت، حاول مجددا.", ephemeral: true });
    }
  }
};
