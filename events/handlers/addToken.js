        ...
        try {
          ...
          if (thetokens && thetokens.includes(thetoken)) {
            return interaction.editReply({
              content: `**هذا التوكن موجود بالفعل**`,
            });
          }

          // تسجيل دخول البوت الجديد
          ...
          const invitebot = new ButtonBuilder()
            .setLabel("دعوة البوت")
            .setURL(
              `https://discord.com/api/oauth2/authorize?client_id=${clienter.user.id}&permissions=8&scope=bot`
            )
          ...
          const broadcast_msg =
            db.get(`broadcast_msg_${interaction.guild.id}`) ??
            "لم يتم تحديد رسالة";
          ...
          return interaction.editReply({
            content: `**الرجاء التأكد من توكن البوت أو تفعيل الخيارات الثلاثة الاخيرة من اعدادات البوت**`,
          });
        }
      }
    }
